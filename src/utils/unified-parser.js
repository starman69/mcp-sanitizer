...
class NormalizedString {
   ...
   */
  getOriginal() {
    console.warn('⚠️  SECURITY WARNING: getOriginal() called - use getNormalized() for validation')
...
function parseUnified(input, options = {}) {
  const {
    ...
    strictMode = false,
    allowOriginalAccess = false, // For backwards compatibility (DEPRECATED)
...
function extractNormalized(value) {
  ...
  if (typeof value === 'string') {
    console.warn('⚠️  SECURITY WARNING: Non-normalized string passed to extractNormalized - use parseUnified() first')
...
function wrapValidator(validatorFunction) {
  return function wrappedValidator(input, ...args) {
    ...
    if (isNormalizedString(input)) {
      ...
      console.warn('⚠️  SECURITY WARNING: Validator received non-normalized string - auto-normalizing')
