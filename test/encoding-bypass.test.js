...
describe('Encoding Bypass Security Fixes', () => {
  ...
  describe('Legacy Method Deprecation', () => {
    it('should use secure methods instead of legacy ones', () => {
      ...
      const result = sanitizer.sanitize('%2e%2e%2f', { type: 'file_path' })

      // Security decoder is integrated and blocks the dangerous pattern
      expect(result.blocked).toBe(true)
      // The path is blocked because it contains directory traversal
      expect(result.warnings.some(w => w.includes('blocked pattern'))).toBe(true)
    })
  })
